...
void
DcaTxop::SetTfBeaconAccessGrantCallback (Callback<void> callback)
{ 
  NS_LOG_FUNCTION (this << &callback);
  m_tfBeaconAccessGrantCallback = callback;
}
...
DcaTxop::NotifyAccessGranted (void)
{
  ...
  if (m_currentHdr.IsTFBeacon ())
    {
      m_tfBeaconAccessGrantCallback ();
    }
  if (m_currentHdr.IsTF ())
    {
      NS_LOG_UNCOND ("Callback to ApWifiMac.");
