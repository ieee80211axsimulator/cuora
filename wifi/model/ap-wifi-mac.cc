  ...
  : m_enableBeaconGeneration (false)
{
  ...
  m_tfSent = false;
  m_beaconDca->SetTfBeaconAccessGrantCallback (MakeCallback (&ApWifiMac::TriggerFrameBeaconExpire, this));
...
void
ApWifiMac::SendTriggerFrameBeacon (void) // Sends one TF beacon
{
  NS_LOG_FUNCTION (this);
  /*
  ...
  uint32_t j = GetNScheduled ();
  if (!m_tfSent)
   {
    for (std::list<Mac48Address>::const_iterator i = m_staList.begin (); i != m_staList.end (); i++)
      {
	alloc.insert (std::pair<Mac48Address, uint32_t>(*i, j)); // The correct logic is still to be decided
	j++;
	if (j == 9)
	 {
	   j = GetNScheduled ();
     ...
     m_tfSent = true;
     m_tfBeaconAlloc = alloc;
   }

  MgtTFBeaconHeader beacon; 
  beacon.SetNoSta (alloc.size());
  beacon.SetRUAllocations (alloc);
  beacon.SetTfDuration (GetTfDuration ());
  packet->AddHeader (beacon);

  std::cout<<"Sending TF Beacon, time = "<<Simulator::Now ().GetMicroSeconds ()<<std::endl;
  Time tfBeaconDuration = m_low->CalculateTfBeaconDuration (packet, hdr);
  m_timeToTF = tfBeaconDuration; 
  m_beaconDca->Queue (packet, hdr);
}

void
ApWifiMac::TriggerFrameBeaconExpire ()
{
  m_dcfManager->NotifyMaybeCcaBusyStartNow (Seconds (1)); //Notify the 20MHz dcfManager that you are busy until TF is sent. 
}
...
ApWifiMac::TriggerFrameExpire ()
{
   ...
   Time t1 = CalculateTfDuration ();
   //m_timeToSendBsrAck = Now () + GetSifs () + t1 + GetSifs () + (GetMaxTfSlots () - 1) * GetSlot () + MicroSeconds (440);
   m_timeToSendBsrAck = Now () + t1 + GetSifs () + GetMaxTfSlots () * GetSlot () + MicroSeconds (440);
   std::cout<<"TF will get access at "<<(Now ()).GetMicroSeconds () << std::endl;
   std::cout<<"TF will be received at  "<<(Now () + t1).GetMicroSeconds () << std::endl;
   std::cout<<"q slots will expire at  "<<(Now () + t1 + GetSifs () + (GetMaxTfSlots () - 1) * GetSlot ()).GetMicroSeconds () << std::endl;
   std::cout<<"m_timeToSendBsrAck = "<<m_timeToSendBsrAck.GetMicroSeconds ()<<std::endl;
...
ApWifiMac::SendBsrAck (Mac48Address to, uint32_t ru)
{
  ...
  std::cout<<"Queueing BSR ACK, ru = "<<ru<<" time = "<<Simulator::Now ().GetMicroSeconds () << std::endl;
...
{
  std::cout<<"In ApWifiMac::StartMuModeUplink\n";
...
ApWifiMac::StartMuModeDownlink (void) //starts OFDMA mode in DL
{
  ...
  Time t1 = CalculateTfDuration ();
  m_triggerFrameDownlinkEvent = Simulator::ScheduleNow (&ApWifiMac::SendTriggerFrame, this, 0); //Downlink TF
}
...
ApWifiMac::CalculateTfDuration (void)
{
  ...
  if (m_tfPacketDuration > 0)
   {
     if (GetNScheduled () < 9)
      {
        total = t1 + GetSifs () + (GetMaxTfSlots () - 1) * GetSlot () + MicroSeconds (440) + GetSifs () + MicroSeconds (416) + GetSifs () + t2 + GetSifs () + MicroSeconds (224); 
              //TF -------------> q slots -----------------------------> BSR --------------------------> BSR ACK ------------------------> UL Payload -----> Payload ACK ------>
      }
  ...
  std::cout<<"total time = " << total.GetMicroSeconds () << std::endl;
  std::cout<<"m_tfPacketDuration = "<<m_tfPacketDuration << std::endl;
      ...
      if (hdr->IsTFResponse ())
        {
          ...
          if (resp.GetData () > m_tfPacketDuration)
           {
             m_tfPacketDuration = resp.GetData ();
           } 

	  std::cout<<"(m_timeToSendBsrAck - Now ()) = "<<(m_timeToSendBsrAck - Now ()).GetMicroSeconds () << std::endl;
          Simulator::Schedule (m_timeToSendBsrAck - Now (), &ApWifiMac::SendBsrAck, this, from, resp.GetRu ());
  ...
  if (m_enableBeaconGeneration)
    {
      if (m_enableBeaconJitter)
      ...
      else
        {
          ...
          Simulator::Schedule (MicroSeconds(990000), &ApWifiMac::SendTriggerFrameBeacon, this);
          Simulator::Schedule (MicroSeconds(999900), &ApWifiMac::StartMuModeUplink, this);
          //m_triggerFrameDownlinkEvent = Simulator::Schedule (Seconds(1.12345), &ApWifiMac::StartMuModeDownlink, this);
        }
    }
